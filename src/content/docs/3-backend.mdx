...
В рамках практической части создадим карту индекса качества городской среды по базе данных SQLite с бэкендом на Python. При желании посмотрите [полный код](https://github.com/gtitov/flask-maplibre-map) и [возможный результат](https://gtitov.github.io/flask-maplibre-map/).
...
<Card title='За что обычно отвечает первый блок <code>.then</code> в цепочке <code>fetch...then</code> при запросе данных с сервера?'>
    ...
    <details>
      ...
      В первом блоке `then` нам доступен специальный объект ответа (Response), но мы ещё не имеем доступа к данным, поэтому мы должны извлечь их, например, методом `response.json()` или `response.text()`.
    </details>
</Card>
...
1. Формируем ответ и отправляем с сервера на клиентскую часть, в браузер. Так как полученный ответ имеет формат GeoJSON, он подаётся на вход карте в качестве источника данных сразу в противовес предыдущему упражнению, где данные от внешнего API приходится [трансформировать](/2-api/#преобразование-данных), прежде чем использовать как источник данных для карты.
...
1. Почему `<year>` пишется в угловых скобках в конструкции `@app.route("/cities/<year>")`?
...
1. Безопасность веб-приложений и распространённые атаки / Дока [[↗]](https://doka.guide/tools/web-security/)