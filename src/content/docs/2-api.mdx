...
<Card title={'На нашей карте в слое <code>clusters</code> что происходит с точками, которые не образуют в кластеры'}>

    <MultipleChoice>

      <Option>
        они не показываются на карте
      </Option>

      <Option isCorrect>
         они показываются, им присваивается радиус по умолчанию
      </Option>

      <Option>
        они показываются, им присваивается радиус и цвет по умолчанию
      </Option>
    ...
    <details>
      <summary>Узнать ответ</summary>
      Они показываются. Радиус получает значение по умолчанию, так как по коду зависит от атрибута `point_count`, а у точек, которые не образуют кластеры, этого атрибута нет. Предупреждение об этом можно увидеть в консоли браузера. Цвет задаётся постоянным значением для всех точек вне зависимости от атрибутов.
    </details>
      ...
      if (f.properties.cluster) {
        map.getSource("vacancies").getClusterLeaves(
            ...
            clusterFeatures.map((feature) => {
              const itemContainer = document.createElement("div"); // императивное создание HTML-элементов
              itemContainer.className = "list-item";
        ...
        document.getElementById("list-selected")
          ...
          </div><hr>` // декларативное создание HTML-элементов
      }
    })
  })
})
